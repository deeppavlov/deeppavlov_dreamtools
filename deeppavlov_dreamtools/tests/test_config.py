...
from deeppavlov_dreamtools.distconfigs.manager import DreamDist, DreamPipeline
...
def test_add_service_to_dreampipeline(dream_root_dir: Path):
    ...
    """
    dream_dist_deepy = DreamDist.from_name(name="deepy_adv", dream_root=dream_root_dir, compose_dev=False,
                                           compose_local=False, compose_proxy=False)
    dream_dist_dream = DreamDist.from_name(name="dream", dream_root=dream_root_dir, compose_local=False)

    pipeline_conf_service_ner = dream_dist_dream.pipeline_conf.config.services.annotators["ner"]
    dream_dist_deepy.pipeline_conf.add_service(name="ner",
    ...
    assert deepy_adv_ner, "Service `ner` couldn't be added via " \
                          "`add_service` method"
