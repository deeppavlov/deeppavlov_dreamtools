...
def test_items_in_dream_dist_is_DreamDist_objects(list_of_dream_dist):
    for dreamdist_item in list_of_dream_dist:
        assert isinstance(
            dreamdist_item, DreamDist
        ), f"{type(dreamdist_item)=} and {type(DreamDist)=}"


def test_length_of_dreamdist_list_matches_the_number_of_directories(
    list_of_dream_dist, dream_assistant_dist_dir
):
