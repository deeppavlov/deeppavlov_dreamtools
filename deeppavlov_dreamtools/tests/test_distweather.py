...
def dream_weather_dist_dir():
    dream_path_dist = Path(Path().absolute().parents[2]) / "dream" / "assistant_dists" / "dream_weather"
...
def test_dream_weather_dist_corresponds_ground_truth_files(file: str, dream_weather_dist_dir) -> None:
    ...
    :param dream_weather_dist_dir: path to root Dream directory
    :return:
    """
    config_path = dream_weather_dist_dir / file
    ground_truth_path = Path(Path().absolute().parent) / "static" / "dream_weather_dist_configs" / file
